...
class Grad():#optim.Optimizer
    ...
    def pc_backward(self,objectives_i,objectives_j):###n
        '''
        ...
        # print(grads_i)
        # for e in range(n):
        #  print("grads_i = ",grads_i,type(grads_i))
        grad = self.comput_gradients_1(grads_i,grads_j )
        #  print("grad = ",grad,type(grad))
            # grads_i = [grad]
            # grads_j = grads_j
    ...
    def comput_gradients_1(self,grad_i,grad_j):
        ...
        # print('两向量间夹角为：', angle * 180.0 / np.pi)
    
        moj_cos = float(grad_j.dot(grad_i)) / float(mo_i)
        # print('向量1在向量2方向上的投影：', moj_cos)

        gradj_i = moj_cos*(1.0/mo_i)*grad_i
        # print(gradj_i)
        gradj_x = grad_j-gradj_i
        # print("向量j在向量i上的分向量是:",j_i,"\n向量j在向量i的垂直方向的分向量是：",j_x)
    ...
    def pack_grad(self, objectives):
        ...
        shapes.append(shape)
        print("111111111111111111111",grad,shape)
...
if  __name__ == '__main__':
    ...
    # i_1 = grad.Judgment_gradient(grad_i=x,grad_j=y)
    # print(i_1)

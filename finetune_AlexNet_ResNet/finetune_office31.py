...
import sys
sys.path.append('../..')
from Vector_Projection_circulate import Grad
...
parser.add_argument('--data', type=str, default='/root/Test/czh/GradMF/finetune_AlexNet_ResNet/data/office31')
...
def finetune(model, model2, dataloaders, optimizer):
    since = time.time()
    best_acc = 0
    ...
    for epoch in range(1, args.n_epoch + 1):
        ...
        for phase in ['src', 'val', 'tar']:
            ...
            for inputs, labelsA in dataloaders[phase]:#源域进行训练
                inputs, labelsA = inputs.to(DEVICE), labelsA.to(DEVICE)
               
                optimizer.zero_grad()
                # with torch.set_grad_enabled(phase == 'src'):
                outputs = model(inputs)
                loss_A = criterion(outputs, labelsA)
                # print(outputs.shape)
                preds = torch.max(outputs, 1)[1]
                if phase == 'src':
                    loss_A.backward()
                    optimizer.step()
                total_loss += loss_A.item() * inputs.size(0)
                # print(preds.shape,labelsA.data.shape)
                correct += torch.sum(preds == labelsA.data)

            if phase =='tar':
                            
                for inputs, labels in dataloaders[phase]:#源域loss
                    inputs, labels = inputs.to(DEVICE), labels.to(DEVICE)
                
                    optimizer.zero_grad()
                    # with torch.set_grad_enabled(phase == 'src'):
                    outputs2 = model2(inputs)
                    loss_A_2 = criterion(outputs2, labels)
                    # if phase == 'src':
                model.train()
                for inputs, labels in dataloaders["tar"]:
                    inputs, labels = inputs.to(DEVICE), labels.to(DEVICE)
                    optimizer.zero_grad()
                    # with torch.set_grad_enabled(phase == 'tar'):
                    outputs = model2(inputs)
                    loss_B = criterion(outputs, labels)

                    Grad_optimizer = Grad(optimizer)
                    Grad_optimizer.zero_grad
                    Grad_optimizer.pc_backward(loss_A_2,loss_B,epoch)
...
if __name__ == '__main__':
    ...
    model2 = load_model(model_name).to(DEVICE)
    print('Source: {} ({}), target: {} ({}), model: {}'.format(
        domain['src'], len(dataloaders['src'].dataset), domain['tar'], len(dataloaders['val'].dataset), model_name))
    optimizer = torch.optim.SGD(model.parameters(), lr=1e-4, momentum = 0.9 )#get_optimizer(model_name)
    model_best, best_acc = finetune(model,model2, dataloaders, optimizer)

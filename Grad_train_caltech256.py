...
def load_model(name='alexnet'):
    if name == 'alexnet':
        ...
        fc = torch.nn.Linear(n_features, 128)
        model.classifier[6] = fc
    elif name == 'resnet':
        model = torchvision.models.resnet50(pretrained=True)
        n_features = model.fc.in_features
        fc = torch.nn.Linear(n_features, 128)#31类，最后全连接层
        model.fc = fc
...
train_A,test_A = load_data(root_path ='/root/Test/czh/GradMF/data/caltech256_A/',batch_size = 32,phase="Src")
train_B,test_B =load_data(root_path ='/root/Test/czh/GradMF/data/caltech256_B/',batch_size = 32,phase='Tar')
...
for epoch in range(Epoch): #训练五十轮
    ...
    if epoch%2 != 0 or epoch==0:
        ...
        if os.path.exists("./Model/Model_Caltech256/best_model.pth")==True:
            model.load_state_dict(torch.load("./Model/Model_Caltech256/best_model.pth"))
        ...
        if test_acc > best_accuracy_A:
            ...
            torch.save(model.state_dict(), './Model/Model_Caltech256/best_model.pth')
        ...
        model2.load_state_dict(torch.load("./Model/Model_Caltech256/best_model.pth"))
